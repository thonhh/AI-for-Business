...
{
	/// <summary>
	/// Summary description for Class_Tim_kiem_tot_nhat_dau_tien.
	/// </summary>
	public class Class_Tim_kiem_tot_nhat_dau_tien
	{
		...
		private int[] truoc;
		private int n0,n1;
		public Class_Tim_kiem_tot_nhat_dau_tien()
		{
			//
		...
		public void Tim_kiem_tot_nhat_dau_tien()
		{
			Khoi_tao();
			bool tim_duoc=false;
			...
			while (!Open.Is_empty())
			{
				...
				int i1,j,k;
				//sap xep Stack tang dan theo h
				for (i1=1;i1<Open.Top;i1++)
				{
					j=i1;
					for (k=i1+1;k<=Open.Top;k++)
						if (Class_MienDoThi.Get_Heuristic(Open.S[k])<Class_MienDoThi.Get_Heuristic(Open.S[j]))
							j=k;
					int temp=Open.S[i1];
					Open.S[i1]=Open.S[j];
					Open.S[j]=temp;
			...
			if (tim_duoc)
			{
				richTextBox_Kq.Text="Đường đi theo BFS : ";
				inkq(n1);
			}
			else
