...
{
	/// <summary>
	/// Summary description for Class_Tim_kiem_theo_AT.
	/// </summary>
	public class Class_Tim_kiem_theo_AT
	{
		...
		public void Tim_kiem_theo_AT()
		{
			...
			while (!Open.Is_empty())
			{
				...
				//chuyen gia tri Min len dinh Stack
				...
				for (int i=1;i<=Class_MienDoThi.dinh_Max;i++)
				{
					if (Class_MienDoThi.Kiem_tra_dinh_thuoc_DThi(i)&& (Class_MienDoThi.Kiem_tra_cung_thuoc_DThi(n,i)))
					{
						...
						else if ((!Close.In_Stack(i))&&(g[i]>g[n]+Class_MienDoThi.Get_Trong_so_Cung(n,i)))
						{
							g[i]=g[n]+Class_MienDoThi.Get_Trong_so_Cung(n,i);
							truoc[i]=n;	
						}
			...
			if (tim_duoc)
			{
				richTextBox_Kq.Text="Đường đi ngắn nhất (theo AT) : ";
				inkq(n1);
				richTextBox_Kq.Text+=" chi phí g= "+g[n1].ToString();
			}
			else
