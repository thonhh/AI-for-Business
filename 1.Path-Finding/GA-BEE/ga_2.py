...
class Chromosome:
    ...
    def _random(self, lower, upper):
        ...
        for i in range(len(lower)):
            ...
            self._genes.append(lower[i] + random.random() * (upper[i] - lower[i]))
...
class GeneticAlgorithm:
    ...
    def _crossover_chromosomes(chromosome1,chromosome2):
        ...
        crossover_chrom._fitness = evaluator(crossover_chrom.get_genes())
    ...
    def _mutate_chromosome(chromosome):
        ...
        for i in range(POPULATION_SIZE):
            if random.random() < MUTATITION_RATE:
                ...
                chromosome._fitness = evaluator(chromosome.get_genes())
...
def evaluator(vector, a=1, b=100):
    ...
    for i in range(vector.__len__()):
        ...
        #    self._fitness += 1
    return fitness#(a - vector[0])**2 + b * (vector[1] - vector[0]**2)**2 #ham can xem lai
